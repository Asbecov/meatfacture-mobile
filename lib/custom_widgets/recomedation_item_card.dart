...
class RecomendationsItemCard extends StatefulWidget {
  const RecomendationsItemCard({
    Key key,
  ...
  @override
  State<RecomendationsItemCard> createState() => _RecomendationsItemState();
}

class _RecomendationsItemState extends State<RecomendationsItemCard> {
  @override
  Widget build(BuildContext context) =>
      BlocBuilder<BasketListBloc, BasketState>(
        builder: (context, state) {
          if (state is BasketLoadedState &&
              state.basketListModel.data.isNotEmpty) {
            widget.recomendation.quantityInClientCart = state
                .basketListModel.data
                .where((element) =>
                    element.assortment.uuid == widget.recomendation.uuid)
                .length
                .toDouble();
          }
          ...
          return Column(
            children: <Widget>[
              AspectRatio(
                ...
                child: ClipRRect(
                  ...
                  child: widget.recomendation.images.isNotEmpty
                      ? CachedNetworkImage(
                          imageUrl: widget
                              .recomendation.images[0].thumbnails.the1000X1000,
                          fit: BoxFit.cover,
              ...
              SizedBox(
                ...
                child: Text(
                  widget.recomendation.name != null
                      ? '${widget.recomendation.name}'
                      : "",
                  maxLines: 2,
  ...
  void _handleOnTap() async {
    ...
    if (await loadToken() != "guest") {
      if (widget.recomendation.assortmentUnitId != "kilogram") {
        if (widget.recomendation.quantityInClientCart <
            widget.recomendation.productsQuantity) {
        ...
        if (widget.recomendation.quantityInClientCart <
          ...
          if (await BasketProvider().updateProductInBasket(
              productUuid: widget.recomendation.uuid,
              quantity: widget.recomendation.quantityInClientCart)) {
            context.read<BasketListBloc>().add(BasketLoadEvent());
          }
        }
      }
  ...
  Widget get _effectiveButton =>
      widget.recomendation.quantityInClientCart == null ||
